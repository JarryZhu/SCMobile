...
+ (MBSpinningCircle*) circleWithSize:(NSSpinningCircleSize)size color:(UIColor*)color
{
...
#else
    MBSpinningCircle *circle = [[[MBSpinningCircle alloc] initWithFrame:CGRectMake(0, 0, width, width)] autorelease];
#endif
    circle.color = color;
    ...
    circle.circleSize = size;
    circle.hasGlow = YES;
    circle.isAnimating = NO;
    circle.speed = 0.55;
    circle.alpha = 0.0;
    return circle;
}
...
- (CGGradientRef) gradient
{
    ...
    CGGradientRef gradientRef = CGGradientCreateWithColors(colorSpace, (ARC_BRIDGE CFArrayRef)gradientColors, gradientLocations);
