...
#pragma mark -- SCRefreshTableHeader private

@interface SCRefreshTableHeader()
...
#pragma mark -- SCRefreshTableHeader 

@implementation  SCRefreshTableHeader
...
- (UILabel *)lastUpdatedLabel
{
    if (!_lastUpdatedLabel) 
    {
		...
		_lastUpdatedLabel.font              = SYSTEMFONT(12.0f);
...
- (UILabel *) statusLabel
{
    if (!_statusLabel) 
    {
		...
		_statusLabel.font               = BOLDSYSTEMFONT(13.0f); //[UIFont fontWithName:@"Times New Roman" size:13.0f];
...
- (void)layoutItems
{
    ...
    if (eTypeHeader == _viewType) 
    {
        ...
        _arrowImage.frame           = CGRectMake(5.0f, rect.size.height - 50.f,  60.0f, 40.0f);
...
- (void)setState:(eRefreshAndReloadState)aState
{
    switch (aState) 
    {
        ...
        {
            _statusLabel.text = NSLocalizedString(@"松开开始加载...",@"");
            _arrowImage.hidden = NO;
            _activityView.hidden = YES;
            [CATransaction begin];
            [CATransaction setAnimationDuration:.18];
